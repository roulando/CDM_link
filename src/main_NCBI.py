...
from src.data.datasets import NCBIdataset, NCBItrainlookup
from src.data.ontology import OntologieNCBI
...
def main(args):
    ...
    onto = OntologieNCBI(read_json(args.onto_path))
    onto.delete_duplicate()

    test_set = NCBIdataset(read_json(args.test_path))
    
    if not args.train_path is None:
        log_prefix+= ":add_train_set"
        train_lookup = NCBItrainlookup(read_json(args.train_path),onto)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--onto_path", type=str, default="./data/ncbi/onto.json")
    parser.add_argument("--train_path", type=str, default="./data/ncbi/train.json")
    parser.add_argument("--test_path", type=str, default="./data/ncbi/test.json")
