...
from src.data.datasets import BC5CDRdataset, BC5CDRtrainlookup
from src.data.ontology import OntologieBC5CDR
...
def main(args):
    ...
    log_prefix = f"BC5CDR-{args.type}"
    log_prefix+= ":"+args.dist_fun

    onto = OntologieBC5CDR(read_json(args.onto_path))
    onto.delete_duplicate()

    with open(args.test_path, 'rb') as f:
        test_set = BC5CDRdataset(pickle.load(f), type=args.type)

    if not args.train_path is None:
        log_prefix+= ":add_train_set"
        with open(args.train_path, 'rb') as f:
            train_lookup = BC5CDRtrainlookup(pickle.load(f), type=args.type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type=str, default="d", help="c or d")
    parser.add_argument("--onto_path", type=str, default="./data/bc5dr/onto.json")
    parser.add_argument("--train_path", type=str, default="./data/bc5dr/train.pkl")
    parser.add_argument("--test_path", type=str, default="./data/bc5dr/test.pkl")
